...
pred.only_summary <- pred.only_annot %>% group_by(annotation) %>% summarise(count=n(), perc=(n()/nrow(pred.only_annot))*100) %>%
  mutate(DApeak='pred')

real.only_annot$annotation <- gsub("\\(.*?\\)", "", real.only_annot$annotation)
real.only_annot$annotation <- trimws(real.only_annot$annotation)
real.only_summary <- real.only_annot %>% group_by(annotation) %>% summarise(count=n(), perc=(n()/nrow(real.only_annot))*100) %>%
  mutate(DApeak='real')

pred.real_annot$annotation <- gsub("\\(.*?\\)", "", pred.real_annot$annotation)
pred.real_annot$annotation <- trimws(pred.real_annot$annotation)
pred.real_summary <- pred.real_annot %>% group_by(annotation) %>% summarise(count=n(), perc=(n()/nrow(pred.real_annot))*100)  %>%
